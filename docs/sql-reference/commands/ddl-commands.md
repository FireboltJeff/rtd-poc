...
| <p><code>SCALE =</code></p><p><code>&#x3C;scale></code></p> | <p>Valid scale numbers include any <code>INT</code> between 1 to 128.</p><p><code>ABORT</code> is an optional parameter (default=false):</p><p><code>ABORT=FALSE</code> means that currently running queries aren’t aborted. The old engine only terminates once the new engine scale is ready, and the running queries are complete.</p><p><code>ABORT=TRUE</code> means that once the new engine is ready, the old engine is terminated, and running queries in it are aborted.</p> | N              |
| <p><code>SPEC =</code></p><p><code>&#x3C;spec></code></p>   | <p>Indicates the EC2 instance type, for example, 'm5.xlarge'</p><p><code>ABORT</code> is an optional parameter (default=false):</p><p><code>ABORT=FALSE</code> means that currently running queries aren’t aborted. The old engine only terminates once the new engine scale is ready, and the running queries are complete.</p><p><code>ABORT=TRUE</code> means that once the new engine is ready, the old engine is terminated, and running queries in it are aborted.</p>          | N              |
